...
points(km_cars$centroids, pch = 22, bg = c(1, 2), cex = 2)
...
##  - Each cell contains frequency of instances classified in a certain way
...
##      = sqrt(1/N * SUM((y[i]-yhat[i])^2))
##          where y[i] = actual outcome for obs i
##          yhat[i] = predicted outcome for obs i
##          N = number of observations
##      - Mean distance between estimates and regression line

## Clustering
##  - Performance measure consists of 2 elements:
##      - Within-cluster similarity: Similarity within each cluster
##          - Within-cluster sum of squares (WSS)
##          - Diameter
##      - Between-cluster similarity: Similarity between clusters
##          - Between-cluster sum of squares (BSS)
##          - Interclsuter distance

## Dunn's Index
##  = Minimal intercluster distance / maximal diameter

##################################################
## The Conflusion Matrix
##################################################
...
rmse <- sqrt(1/dim(air)[1] * sum((air$dec-pred)*(air$dec-pred)))
...
for (i in 1:6) {
    ...
    accs[i] <- (conf[1] + conf[4]) / (conf[1] + conf[2] + conf[3] + conf[4])
...
##  - Irreducible error: Noise - don't minimize
##  - Reducible error: Error due to unfit model - minimize
...
##      - e.g., linear model for quadratic data

## Variance
##  - Error due to variance: Error due to the sampling of the training set
##  - Model with high variance fits trianing set closely
##  - Few restrictions lead to high variance
##      - e.g., polynomial model for quadratic data
##  - High variance generalizes bad to test set

## Overfitting
##  - Accuracy will depend on dataset split
##  - High variance will heavily depend on split
##  - Overfitting = model fits training set a lot better than test set
##  - Too specific

## Underfitting
##  - Restricting model too much
##  - High bias
##  - Too general
...
## High interpretability usually means high bias.
...
##  - Horizontal axis: False positive rate (FPR) = FP / (FP+TN)
##  - Vertical axis: True positive rate (TPR) = recall
##  - Closer to upperleft corner, the better
##  - Good classifiers have big area under curve (AUC)
...
# Compare decision tree model and k nearest neighbors model
...
# Draw the ROC lines using predefined draw_roc_lines()
...
# The kang_nose dataset and nose_width_new are already loaded 
# in your workspace.

# Plot nose length as function of nose width.
plot(kang_nose$nose_length ~ kang_nose$nose_width)
...
## R^2 = 1 - (SS[res]/SS[tot])
##  where SS[res] = SUM(res[i]^2) = SUM((y[i]-yhat[i])^2)
##  SS[tot] = SUM((y[i]-ybar[i])^2)
##      ybar[i] = sample mean of the response
## The closer R^2 to 1, the greater the degree of linear association
##  between the predictor and the response variable

# kang_nose, lm_kang and res are already loaded in your workspace

# Calculate the residual sum of squares: ss_res
pred <- predict(lm_kang, kang_nose)
ss_res <- sum((kang_nose$nose_length-pred)^2)

# Determine the total sum of squares: ss_tot
mean <- mean(kang_nose$nose_length)
ss_tot <- sum((kang_nose$nose_length-mean)^2)
...
## Higher R-squared value means better prediction.
##  - Explains higher amount of variance.
...
##          - e.g., plot(lm_shop$fitted.values, lm_shop$residuals)
##      - Identical Normal: Approximately a line?
##          - e.g., qqnorm(lm_shop$residuals)
...
##      - 2. Select k nearest
##      - 3. Aggregate the responses to the k nearest
##      - 4. The outcome is your prediction
##  - Choosing k (Bias-Variance Tradeoff)
##      - k=1: Perfect fit on training set but poor predictions
##      - k=N: Mean, also poor predictions
##      - Reasonable k: 20% of N

## Generalization in Regression
##  - 1. Hold Out
##      - 1) Split train/test
##      - 2) Build regresion model on training set
##      - 3) Calculate RMSE within training set
##      - 4) Predict the outcome of the test set
##      - 5) Calculate RMSE within test set
##      - 6) Compare test RMSE with train RMSE
...
# x_pred: predictor values of the new observations (world_bank_test$cgdp)
# x: predictor values of the training set (world_bank_train$cgdp)
# y: corresponding response values of training set (world_bank_train$urb_pop)
...
my_knn <- function(x_pred, x, y, k) {
    ...
    return(predict_knn) # predicted values for new observations
}
###
...
test_output <- my_knn(world_bank_test$cgdp,
                      world_bank_train$cgdp, world_bank_train$urb_pop, 30)
...
lines(world_bank_test$cgdp[ranks], test_output_lm[ranks], 
      lwd = 2, col = "blue")

# Predict with log-linear model and add line
test_output_lm_log <- predict(lm_wb_log, data.frame(cgdp=world_bank_test$cgdp))
lines(world_bank_test$cgdp[ranks], test_output_lm_log[ranks], 
      lwd = 2, col = "red")

# Predict with k-NN and add line
test_output_knn <- my_knn(world_bank_test$cgdp, 
                          world_bank_train$cgdp, world_bank_train$urb_pop, 30)
lines(world_bank_test$cgdp[ranks], test_output_knn[ranks], 
      lwd = 2, col = "green")
...
##  - 3. Move centroids to average location
##  - 4. Repeat steps 2 and 3 until conversion
...
##  - Problems: WSS keeps decreasing as k increases
##  - Solution: WSS starts decreasingly slowly
##      - WSS / TSS < 0.2
##      - TSS = WSS + BSS
##  - Scree Plot
##      - Visualize the ratio of WSS/TSS as a function of k
##      - Look for the elbow in the plot

## k-Means in R
##  - my_km <- kmeans(data, centers, nstart)
##      - centers: starting centroid or # of clusters
##      - nstart: # of times R restarts with different centroids
##      - default distance measure is Euclidean metric
...
seeds_km_2 <- kmeans(seeds, 5, nstart=2)
...
##      - Separation betewen clusters
## - Alternative:
##      - Intracluster Diameter
##          - Max distance between each pair of objects within cluster
##              - Measure of compactness
##      - Intercluster Distance
##          - Min distance between any pair of objects between cluster
##              - Measure of separation

## Dunn's Index
##  - Intercluster Distance / Intracluster Diameter
##  - Higher Dunn means better separated, more compact clusters
...
plot(run_record$marathon, run_record$X100m, col=run_km$cluster,
     xlab="Marathon", ylab="X100m", main="Marathon vs. X100m")
...
# Cluster run_record_sc using k-means: run_km_sc. 
# 5 groups, let R start over 20 times
run_km_sc <- kmeans(run_record_sc, 5, nstart=20)

# Plot records on 100m as function of the marathon. 
# Color using the clusters in run_km_sc
...
##      - May lead to undesirable chaining effect
...
##  - Pros:
##      - In-depth analysis
##      - Different linkage methods -> different patterns
##  - Cons:
##      - High computational cost
...
# Apply rect.hclust() on run_single to draw the boxes around 5 clusters
